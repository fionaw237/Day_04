...
class TestFriends < MiniTest::Test
  ...
  def test_add_friend()
    ...
    assert_equal(@person2[:friends].length(), original_length + 1)
  end

  # 5. For a given person, remove a specific name from their list of friends
  # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_remove_friend()
    original_length = @person4[:friends].length()
    remove_friend(@person4, "Velma")
    assert_equal(@person4[:friends].length(), original_length - 1)
  end
  ...
  def test_loan_money()
    lender_orig_balance = @person5[:monies]
    lendee_orig_balance = @person4[:monies]
    loan_money(@person5, @person4, 20)

    assert_equal(lender_orig_balance - 20, @person5[:monies])
    assert_equal(lendee_orig_balance + 20, @person4[:monies])
  end
  ...
  def test_favourite_food()
    ...
    expected_array = ["charcuterie", "soup", "bread", "Scooby snacks", "spaghetti", "ratatouille", "spinach"]
